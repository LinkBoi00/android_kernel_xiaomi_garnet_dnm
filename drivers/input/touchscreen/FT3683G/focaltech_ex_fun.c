...
static ssize_t fts_lockdown_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
...
/* N17 code for HQ-291087 by liunianliang at 2023/5/29 start */
static DEVICE_ATTR(fts_lockdown_info, S_IRUGO | S_IWUSR, fts_lockdown_show, NULL);
/* N17 code for HQ-291087 by liunianliang at 2023/5/29 end */
...
static int fts_tp_info_show(struct seq_file *m, void *v)
{
    ...
    if ((ret < 0) || (fwver == 0xFF) || (fwver == 0x00))
        snprintf(buf, sizeof(buf), "get tp info fail, please resume TP!");
    else
        snprintf(buf, sizeof(buf), "[Vendor]:%s [TP-IC]:FT3683G [FW]:0x%02x [VID]:0x%02x",
...
int fts_ts_hw_info(struct fts_ts_data *ts_data)
{
    ...
    if ((ret < 0) || (fwver == 0xFF) || (fwver == 0x00))
        ...
        snprintf(buf, sizeof(buf), "[Vendor]:%s [TP-IC]:FT3683G [FW]:0x%02x", ts_data->vendor, fwver);
...
int fts_create_procfs(struct fts_ts_data *ts_data)
{
    ...
    /* create /proc/tp_info */
    tp_info_proc = proc_create("tp_info", 0664, NULL,
        &fts_tp_info_ops);
    if (!tp_info_proc) {
    ...
    tp_data_dump_proc = proc_create("tp_data_dump", 0664, NULL,
        &fts_tp_data_dump_ops);
    ...
    tp_ito_test_proc = proc_create("self_test", 0664,
        tp_android_touch_proc, &fts_ito_test_ops);
    if (!tp_ito_test_proc) {
