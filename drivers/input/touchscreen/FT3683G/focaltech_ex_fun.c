...
static ssize_t fts_charger_mode_write(
	struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
...
int fts_create_apk_debug_channel(struct fts_ts_data *ts_data)
{
	...
	ts_data->proc_charger.proc_entry = proc_create_data("TP_charger_flags", 0777, NULL, \
								  &fts_charger_fops, ts_data);
...
static ssize_t fts_vendor_id_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	int count = 0;
	u8 val = 0;
...
static void fts_diff_get_data(u8* all_data_buf, int buf_len)
{
	...
	do {
		...
		if (ret < 0) {
			FTS_ERROR("diff_data_buf[0](%x) abnormal,ret:%d", all_data_buf[0], ret);
		}
		if ((temp_count != all_data_buf[90]) && (all_data_buf[90]!= 0xFF)) {
			temp_count = all_data_buf[90];
			break;
		}
		FTS_ERROR("diff_count: %d  temp_count:%d cnt:%d", all_data_buf[90], temp_count, cnt);
...
static int fts_diff_data_output(char *buf, u8* all_data_buf)
{
	...
	for (j = 0; j < FTS_TX_MAX; j++) {
		for (i = 0; i < FTS_RX_MAX; i++) {
			count += snprintf(buf + count, PAGE_SIZE, "%4d ", diff_data[j*FTS_RX_MAX + i]);
		}
		count += snprintf(buf + count, PAGE_SIZE, "\n");
	}
	for (j = FTS_TX_MAX*FTS_RX_MAX; j < FTS_DIFF_DATA_LEN/2; j++) {
		...
		|| (j == (FTS_TX_MAX*FTS_RX_MAX + FTS_RX_MAX + FTS_TX_MAX - 1))
		|| (j == (FTS_TX_MAX*FTS_RX_MAX + FTS_RX_MAX + FTS_TX_MAX + 11 - 1))
		|| (j == (FTS_TX_MAX*FTS_RX_MAX + FTS_RX_MAX + FTS_TX_MAX + 11 + FTS_RX_MAX- 1))
		|| (j == (FTS_TX_MAX*FTS_RX_MAX + FTS_RX_MAX + FTS_TX_MAX + 11 + FTS_RX_MAX + FTS_TX_MAX - 1))
		|| (j == (FTS_TX_MAX*FTS_RX_MAX + FTS_RX_MAX + FTS_TX_MAX + 11 + FTS_RX_MAX + FTS_TX_MAX + 11 - 1))) {
			count += snprintf(buf + count, PAGE_SIZE, "\n");
		}
	}
	return count;
...
static ssize_t fts_diff_data_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	int count = 0;
	...
	mutex_lock(&ts_data->input_dev->mutex);
	all_data_buf = (u8 *)kzalloc((FTS_DATA_WITH_DIFF_LEN) * sizeof(u8), GFP_KERNEL);
...
static ssize_t fts_fast_diff_mode_store(
	struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct fts_ts_data *ts_data = dev_get_drvdata(dev);
...
static ssize_t fts_fast_diff_mode_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	int count = 0;
	u8 val = 0;
...
static DEVICE_ATTR(fts_fast_diff_mode, S_IRUGO | S_IWUSR, fts_fast_diff_mode_show, fts_fast_diff_mode_store);
