...
#endif
		FTS_INFO("notifier,event:%lu,blank:%d, lcm_now_state = %d", event, blank_value, lcm_now_state);
		if (lcm_now_state == 1) {
			cancel_work_sync(&fts_data->resume_work);
			fts_ts_suspend(ts_data->dev);
		} else if ((blank_enum[1] == blank_value) && (event_enum[1] == event)) {
...
{
	sprintf(current_tp_info.id,"0x%02x%02x", ts_data->ic_info.ids.chip_idh, ts_data->ic_info.ids.chip_idl);
